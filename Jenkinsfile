  ...
  stages {
    ...
    stage('docker build and push') {
      steps {
        ...
        ansible node -m shell -a "sudo docker pull brian24/keduitlab:red"
        '''
      }
    }
    stage('deploy and service') {
      steps {
        sh '''
        ansible master -m shell -a 'sudo kubectl create deploy web-red --replicas=3 --port=80 --image=brian24/keduitlab:red'
